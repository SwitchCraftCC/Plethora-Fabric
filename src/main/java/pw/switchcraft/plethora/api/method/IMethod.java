...
import pw.switchcraft.plethora.api.module.ModuleContainerMethod;
...
public interface IMethod<T> {
	 ...
	 * @see dan200.computercraft.api.lua.IDynamicLuaObject#callMethod(ILuaContext, int, IArguments)
	 */
	@Nonnull
	FutureMethodResult apply(@Nonnull IUnbakedContext<T> context, @Nonnull IArguments args) throws LuaException;
	 ...
	 * purposes. You should override {@link #canApply(IPartialContext)} or use an existing class like
	 * {@link ModuleContainerMethod} to actually enforce constraints.
	 *
	 * @return The modules that this method requires
	 */
	@Nonnull
	default Collection<Identifier> getModules() {
	...
	interface Delegate<T> {
		 ...
		 * @throws LuaException     On the event of an error
		 * @throws RuntimeException Unhandled errors: these will be rethrown as {@link LuaException}s and the call stack logged.
		 * @see IMethod#apply(IUnbakedContext, IArguments)
		 */
		FutureMethodResult apply(IUnbakedContext<T> context, IArguments args) throws LuaException;
	}
}
